...
function startpoint_optionsframework_add_admin() {
    global $query_string;

    $themename = startpoint_get_option('of_themename');
    $shortname = startpoint_get_option('of_shortname');

    if (isset($_REQUEST['page']) && $_REQUEST['page'] == 'optionsframework') {
        if (isset($_REQUEST['of_save']) && 'reset' == $_REQUEST['of_save']) {
            $options = startpoint_get_option('of_template');
            startpoint_reset_options($options, 'optionsframework');
    ...
    $of_page = add_theme_page($themename, 'Theme Options', 'edit_theme_options', 'optionsframework', 'startpoint_optionsframework_options_page', 'div');

    // Add framework functionaily to the head individually
    add_action("admin_print_scripts-$of_page", 'startpoint_load_only');
}

add_action('admin_menu', 'startpoint_optionsframework_add_admin');
/* ----------------------------------------------------------------------------------- */
/* Options Framework Reset Function - of_reset_options */
/* ----------------------------------------------------------------------------------- */

function startpoint_reset_options($options, $page = '') {
    global $wpdb;
    $count = 0;
    ...
    //When Theme Options page is reset - Add the startpoint_of_options option
    if ($page == 'optionsframework') {
        startpoint_delete_option('startpoint_of_options');
    }
}
...
function startpoint_optionsframework_options_page() {
    $options = startpoint_get_option('of_template');
    $themename = startpoint_get_option('of_themename');
    ?>
	<div class="trail-notify">
        <h1>Get StartPoint PRO Theme!</h1>
        <p style="font-size:15px; line-height: 20px;">You are using the StartPoint, Free Version of StartPoint Pro Theme. Upgrade to Pro for extra features like Home Page Unlimited Slider, Unlimited Testimonial Slider, Wookmark Filtered Gallery, Parallax Effect, FullWidth, Grid, List and many more Page Templates, Multiple Color Options, Life time theme support and much more.</p>
        <a href="<?php echo esc_url('http://www.inkthemes.com/wp-themes/single-page-wordpress-theme/'); ?>" target="blank">Upgrade to StartPoint PRO Theme here >></a>
    </div>
    ...
    $return = startpoint_optionsframework_machine($options);
    ?>
            <div id="main">
                <div id="of-nav">
                    <ul>
...
/* ----------------------------------------------------------------------------------- */
/* Ajax Save Action - startpoint_ajax_callback */
/* ----------------------------------------------------------------------------------- */
add_action('wp_ajax_of_ajax_post_action', 'startpoint_ajax_callback');

function startpoint_ajax_callback() {
    ...
    if ($save_type == 'upload') {
        ...
        startpoint_update_option($clickedID, $uploaded_file['url']);
        ...
        foreach ($options as $option_array) {
            $id = $option_array['id'];
            $old_value = startpoint_get_option($id);
            $new_value = '';
                ...
                if (is_array($type)) {
                    foreach ($type as $array) {
                        if ($array['type'] == 'text') {
                            ...
                            startpoint_update_option($id, stripslashes($new_value));
                        }
                    }
                } elseif ($new_value == '' && $type == 'checkbox') { // Checkbox Save
                    startpoint_update_option($id, 'false');
                } elseif ($new_value == 'true' && $type == 'checkbox') { // Checkbox Save
                    startpoint_update_option($id, 'true');
                    ...
                    foreach ($option_options as $options_id => $options_value) {
                        ...
                        if (!isset($output[$multicheck_id])) {
                            startpoint_update_option($multicheck_id, 'false');
                        } else {
                            startpoint_update_option($multicheck_id, 'true');
                        }
                    }
                    ...
                    startpoint_update_option($id, $typography_array);
                } elseif ($type == 'border') {
                    ...
                    startpoint_update_option($id, $border_array);
...
function startpoint_optionsframework_machine($options) {
    ...
    foreach ($options as $value) {
        ...
        switch ($value['type']) {
            ...
            case 'text':
                $val = $value['std'];
                $std = startpoint_get_option($value['id']);
                if ($std != "") {
                    $val = $std;
                }
            ...
            case 'select':
                ...
                $select_value = startpoint_get_option($value['id']);
            ...
            case "upload":
                $value['std'] = '';
                if (isset($value['std'])) {
                    $output .= startpoint_optionsframework_uploader_function($value['id'], $value['std'], null);
                }
                break;
            case "upload_min":

                $output .= startpoint_optionsframework_uploader_function($value['id'], $value['std'], 'min');

                break;
            case "color":
                $val = $value['std'];
                $stored = startpoint_get_option($value['id']);
                if ($stored != "") {
                    $val = $stored;
                }
            ...
            case "typography":
                ...
                $default = $value['std'];
                $typography_stored = startpoint_get_option($value['id']);
            ...
            case "border":
                ...
                $default = $value['std'];
                $border_stored = startpoint_get_option($value['id']);
        ...
        if (is_array($value['type'])) {
            foreach ($value['type'] as $array) {
                $id = $array['id'];
                $std = $array['std'];
                $saved_std = startpoint_get_option($id);
                if ($saved_std != $std) {
                    $std = $saved_std;
                }
                $meta = $array['meta'];
...
/* ----------------------------------------------------------------------------------- */
/* OptionsFramework Uploader - startpoint_optionsframework_uploader_function */
/* ----------------------------------------------------------------------------------- */
